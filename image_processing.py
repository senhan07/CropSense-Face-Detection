...
def draw_rectangle(square_region, \
                    is_error, \
                    output_res, \
                    output_image_path, \
                    image, \
    ...
    thickness = max(resolution_thickness_ratio, 1)
    ...
    # cv2.putText(image, quality, (startX, startY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    
    third_text = ""
    # THIRD TEXT LABEL
    if is_error == False:
       third_text = f"Quality: {quality}%"
    third_font_scale = font_scale
    third_font_thickness = font_thickness

    third_text_size, _ = cv2.getTextSize(third_text, cv2.FONT_HERSHEY_SIMPLEX, third_font_scale, third_font_thickness)

    third_background_width = third_text_size[0] + 10
    third_background_height = third_text_size[1] + 10

    third_background_color = (255, 255, 255)  # White background color
    third_text_color = (0, 0, 0)  # Black text color

    third_background = np.zeros((third_background_height, third_background_width, 3), dtype=np.uint8)
    third_background[:, :] = third_background_color

    if third_background_width > debug_image.shape[1]:
        third_ratio = debug_image.shape[1] / third_background_width
        third_background_width = debug_image.shape[1]
        third_background_height = int(third_background_height * third_ratio)
        third_background = cv2.resize(third_background, (third_background_width, third_background_height))

    cv2.putText(third_background, third_text, (10, third_text_size[1] + 5), cv2.FONT_HERSHEY_SIMPLEX, third_font_scale, third_text_color, third_font_thickness)

    debug_image_height, debug_image_width, _ = debug_image.shape
    combined_height = background_height + second_background_height + third_background_height

    if combined_height > debug_image_height:
        debug_image = cv2.resize(debug_image, (debug_image_width, combined_height))

    debug_image[background_height + second_background_height:combined_height, 0:third_background_width] = third_background
    ...
    return debug_image, resized_image
